...
/**
 * evo_webhook.php — authenticated Evolution API webhook receiver.
 *
 * Validates every request before it can cause anything to happen. Evolution v2
 * does not sign webhooks, so the token in the URL is the authentication and the
 * endpoint must be HTTPS.
 *
 * Contract with Evolution API (v2.3.7):
 *
 *   POST https://crm.dishnetafrica.com/_plugins/dishnet-hybrid-telecom/evo_webhook.php?token=<secret>
...
foreach ($messages as $msg) {
    ...
    try {
        ...
        error_log('[evo_webhook] queue failed: ' . $e->getMessage());
    }
}
